...
class ResultSaver:
    def __init__(self, item_names: list, save_probs: bool = False):
        self._item_names = item_names
        self._results_dict = defaultdict(list)
        self._save_probs = bool(save_probs)
    ...
    def append(self, batch_meta_data: dict,targets:dict, results: dict) -> None:
        ...
        for k in self._item_names:
            ...
            # Save per-class probabilities (for ensemble/calibration).
            # Only for onehot classification tasks.
            if self._save_probs and Config.get_type(k) == "onehot":
                try:
                    prob = res[k]
                    if isinstance(prob, torch.Tensor):
                        # For safety: if shape is [B,C,L], average over time.
                        if prob.dim() > 2:
                            prob = prob.mean(dim=-1)
                        prob = prob.detach().cpu()
                    # prob: [B, C]
                    if isinstance(prob, torch.Tensor) and prob.dim() == 2:
                        c = int(prob.size(1))
                        prob_list = prob.tolist()
                        for ci in range(c):
                            self._results_dict[f"prob_{k}_{ci}"].extend([float(row[ci]) for row in prob_list])
                except Exception as e:
                    if not hasattr(self, "prob_warning_flag"):
                        logger.warning(f"[ResultSaver] failed to save prob columns for {k}: {e}")
                        setattr(self, "prob_warning_flag", 1)
