...
class Metrics:
    ...
    Available metrics: `Precision`, `Recall`, `F1`, `Accuracy`, `Mean`, `Std`, `MAE`, `MAPE`, `RMSE`, `R2`
    """

    _epsilon = 1e-6
    _avl_regr_keys = ("sum_res", "sum_squ_res", "sum_abs_res", "sum_abs_per_res")
    _avl_cmat_keys = ("tp", "predp", "possp")
    _avl_metrics = ("precision", "recall", "f1", "accuracy", "mean", "std", "mae", "mape", "rmse", "r2")
    ...
    def __init__(
        ...
        if "accuracy" in self._metric_names:
            data_keys += ("correct",)
    ...
    def compute(
        ...
        if set(self._metric_names) & set(("precision", "recall", "f1")):
            if self._task in ["ppk", "spk"]:
                ...
                if "accuracy" in self._metric_names:
                    # correct predictions count for one-hot classification
                    self._data["correct"] = torch.sum(targets * preds)
        elif "accuracy" in self._metric_names:
            # Accuracy only makes sense for one-hot classification here.
            assert (
                targets.size() == preds.size()
            ), f"`{targets.size()}` != `{preds.size()}`"
            assert targets.size(-1) > 1, f"The input must be one-hot."
            preds_indices = preds.topk(1).indices
            preds_oh = preds.zero_().scatter_(dim=1, index=preds_indices, value=1)
            targets_indices = targets.topk(1).indices
            targets_oh = targets.zero_().scatter_(dim=1, index=targets_indices, value=1)
            self._data["correct"] = torch.sum(targets_oh * preds_oh)
    ...
    def _update_metric(self, key: str) -> torch.Tensor:
        ...
        if key == "precision":
            ...
            v = self._data["accuracy"] = self._data["correct"] / (self._data["data_size"] + self._epsilon)
        elif key == "mean":
