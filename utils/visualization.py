...
def vis_detection_picking(
    ...
    '''
    visualize earthquake detection and phase picking tasks simultaneously (for EQTransformer and SeisT-family)
    '''
...
def vis_phase_picking(
    ...
    '''
    visualize phase picking task only, not include detection (for PhaseNet and SeisMoLLM)
    '''
    fig = plt.figure(figsize=(10/2.54,10/2.54))
    if sampling_rate is None:
        x = list(range(len(waveforms[0])))
    ...
    plt.ylabel('Probability')
    plt.plot(x,preds[1], f'--C5', linewidth=1, alpha=0.8,label=pred_phase_labels[1])
    plt.ylabel('Probability')
